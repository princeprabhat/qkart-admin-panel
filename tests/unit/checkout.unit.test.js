  ...
  describe("Checkout", () => {
    ...
    it("should throw 404 error if cart is not present", async () => {
      ...
      //  expect(true).toEqual(false);
       ...
       await expect(res).rejects.toEqual(expect.objectContaining({statusCode:httpStatus.NOT_FOUND,message: "User does not have a cart", stack: expect.any(String)}));
    });
    ...
    it("should throw 400 error if user's cart doesn't have any product", async () => {
      ...
      await expect(res).rejects.toThrow(ApiError);
      await expect(res).rejects.toEqual(expect.objectContaining({statusCode:httpStatus.BAD_REQUEST,message:"User's cart doesn't have any product"}));
    });
    ...
    it("should throw 400 error if address is not set - when User.hasSetNonDefaultAddress() returns false", async () => {
      ...
      // - the "statusCode" field of response is "400 BAD REQUEST"
      await expect(res).rejects.toThrow(ApiError);
      await expect(res).rejects.toEqual(expect.objectContaining({statusCode:httpStatus.BAD_REQUEST,message:"Address is not set"}))
    });
    ...
    it("should throw 400 error if wallet balance is insufficient", async () => {
      ...
      await expect(res).rejects.toThrow(ApiError);
      await expect(res).rejects.toEqual(expect.objectContaining({statusCode:httpStatus.BAD_REQUEST,message:"Wallet balance is insufficient"}));
    });
